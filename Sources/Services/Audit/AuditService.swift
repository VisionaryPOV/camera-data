...
public final class AuditService: AuditTracking, @unchecked Sendable {
    ...
    public func recordChanges(
        ...
        let tracked: [(String, String, String)] = [
            ...
            ("filter", before.filter, after.filter),
            ("iso", String(before.iso), String(after.iso)),
            ("fps", String(before.fps), String(after.fps)),
            ("whiteBalance", before.whiteBalance, after.whiteBalance),
            ("shutterAngle", String(before.shutterAngle ?? 0), String(after.shutterAngle ?? 0)),
            ("shutterSpeed", before.shutterSpeed ?? "", after.shutterSpeed ?? ""),
